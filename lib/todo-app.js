...
var translations = {
  en: {
    ...
    itemsLeft: " item left",
    itemsLeftPlural: " items left"
  },
  ru: {
    title: "задачи",
    placeholder: "Что нужно сделать?",
    markAllComplete: "Отметить все как выполненные",
    all: "Все",
    active: "Активные",
    completed: "Выполненные",
    clearCompleted: "Очистить выполненные",
    itemsLeft: " осталась",
    itemsLeftPlural: " осталось",
    itemsLeftMany: " осталось"
  }
};
...
function update(action, model, data) {
  ...
  switch(action) {
    ...
    case 'ROUTE':
      ...
      break;
    case 'CHANGE_LANG':
      new_model.lang = new_model.lang === 'en' ? 'ru' : 'en';
      break;
...
function render_main (model, signal) {
  ...
  return (
    section(["class=main", "id=main", display], [ // hide if no todo items.
      ...
      label(["for=toggle-all"], [ text(t.markAllComplete) ]),
      ul(["class=todo-list"],
...
function render_footer (model, signal) {
  ...
  if (lang === 'ru') {
    ...
    if (lastDigit === 1 && lastTwoDigits !== 11) {
      left = " задача осталась";
    } else if ([2, 3, 4].indexOf(lastDigit) !== -1 && 
               [12, 13, 14].indexOf(lastTwoDigits) === -1) {
      left = " задачи" + t.itemsLeftPlural;
    } else {
      left = " задач" + t.itemsLeftMany;
    }
  } else {
    left = t.itemsLeft;
    if (count > 1 || count === 0) {
      left = t.itemsLeftPlural;
    }
  }
  ...
  return (
    section(["class=todoapp"], [ // array of "child" elements
      header(["class=header"], [
        button([
          "class=lang-switch",
          "style=font-size:1.5em;padding:10px 20px;margin-bottom:10px;",
          typeof signal === 'function' ? signal('CHANGE_LANG') : ''
        ], [
          text(lang === 'en' ? 'English' : 'Русский')
        ]),
        h1([], [
          text(t.title)
        ]), // </h1>
